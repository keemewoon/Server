...
public class Ex05_PreparedStatement {
	...
	public static void main(String[] args) {
		...
		// - 정적쿼리(매개변수가 없는 쿼리):Statement or PreparedStatement 동일 > Statement 사용권장(간단함)
	...
	private static void m3() {
		...
		String sql = "select * from tblAddress where name = ?"; //홍길동이 사용자 입력값?
		...
		try {
			...
			while (rs.next()) {
				System.err.println(rs.getString("name") + "," + rs.getString("address"));
			}
	...
	private static void m2() {
		...
		try {
			...
			//SQL > ? > 오라클 매개변수(오라클 문법) == String format()과 비슷한 역할
			...
			stat.setString(1, name); //1번째 ?(매개변수)에 name의 변수값을 넣어라!
			stat.setInt(2, age);
