		...
		protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			try {
				...
				while (rs.next()) {
					...
					//********jdbc -> 데이터베이스에서 불러온 결과 테이블에서 모든 컬럼을 HashMap에 옮겨담은것
					...
					//map은 지역변수라 밖에있는 array List에 집어넣는다.
					list.add(map);

				}
				//list == rs
				//Result set의 모든 내용을 ArrayList<HashMap>에 복사를 함.
			...
			////3. JSP 호출하기 + ResultSet(ArrayList<HashMap>에 담음)
			//JSP에게 전달한다.
